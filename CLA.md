# platform-workflows CLA Overview

`platform-workflows` 当前采用 [`Business Source License 1.1`](./LICENSE) 分发，并在 `LICENSE` 中声明未来切换到 `GPL-2.0-or-later`。 为了降低外部贡献的授权链风险，本仓库采用分层贡献协议：
...
- `platform-workflows` 不只是一个普通开源仓库；贡献可能被用于当前 `BSL 1.1` 版本、未来 `GPL-2.0-or-later` 版本，以及项目维护者提供的商业授权版本。
...
被接收的贡献可能被用于当前 `BSL 1.1` 版本、未来 `GPL-2.0-or-later` 版本，以及项目维护者提供的商业授权版本。
